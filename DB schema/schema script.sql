...
CREATE TABLE IF NOT EXISTS PUBLISHER_ADDRESS (
    ID        INT NOT NULL,
    STREET    VARCHAR(50) NOT NULL,
    CITY      VARCHAR(50) NOT NULL,
    COUNTRY   VARCHAR(90) NOT NULL,

    PRIMARY KEY(ID, STREET, CITY, COUNTRY),
    FOREIGN KEY(ID) REFERENCES PUBLISHER(ID) ON DELETE CASCADE ON UPDATE CASCADE
);
##
...
# Event used to delete any complete orders made by the bookstore older than 3 months.
CREATE EVENT remove_old_bookstore_orders
ON SCHEDULE EVERY 1 DAY
DO
	DELETE FROM COMPLETE_ORDER WHERE COMPLETE_TIME < NOW() - interval 3 MONTH;
